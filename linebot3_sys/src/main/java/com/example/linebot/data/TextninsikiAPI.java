...
@Repository
public class TextninsikiAPI {
    // あなたのEC2の画像の送付先 application.propertiesファイルから読み込まれる
    @Value("${textninsiki.api.url}")
    ...
    public TextninsikiAPI(RestTemplateBuilder restTemplateBuilder) {
        this.restTemplate = restTemplateBuilder.build();
    }

    public TextResponse go(Resource imageResource) {
        ...
        ResponseEntity<TextResponse> response = restTemplate.postForEntity(API_URL, formEntity, TextResponse.class);
